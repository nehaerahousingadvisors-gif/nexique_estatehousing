...
function toProject(docId: string, d: Record<string, any>, i: number): FProject {
  ...
  return {
    ...
    details: [
      { label: 'Inventory Type',       value: d.inventoryType || d.propertyType || '' },
      { label: 'Project',              value: d.projectName   || '' },
      { label: 'Developer',            value: d.developer || d.developerName || '' },
      { label: 'Location',             value: d.projectLocation || d.location || '' },
      { label: 'City',                 value: d.city          || '' },
      { label: 'Looking To',           value: d.lookingTo     || '' },
      { label: 'Project Land Area',    value: d.landArea      || '' },
      { label: 'Total Towers',         value: d.totalTowers   || '' },
      { label: 'Total Residences',     value: d.totalResidences || '' },
      { label: 'RERA Number',          value: d.reraNumber    || '' },
      { label: 'Status',               value: d.availability || d.status || '' },
      // Residential
      { label: 'Bedrooms',             value: d.bedrooms != null && d.bedrooms !== '' ? String(d.bedrooms) : '' },
      { label: 'Bathrooms',            value: d.bathrooms != null && d.bathrooms !== '' ? String(d.bathrooms) : '' },
      { label: 'Balconies',            value: d.balconies != null && d.balconies !== '' ? String(d.balconies) : '' },
      { label: 'Area',                 value: d.area || '' },
      { label: 'Plot Area',            value: d.plotArea ? `${d.plotArea} ${d.plotUnit || 'sq.ft.'}` : '' },
      { label: 'Carpet Area',          value: d.carpetArea ? `${d.carpetArea} sq.ft.` : '' },
      { label: 'Built-up Area',        value: d.builtUpArea ? `${d.builtUpArea} sq.ft.` : '' },
      { label: 'Super Built-up Area',  value: d.superBuiltUpArea ? `${d.superBuiltUpArea} sq.ft.` : '' },
      { label: 'Furnishing',           value: d.furnishing    || '' },
      { label: 'Covered Parking',      value: d.coveredParking != null && d.coveredParking > 0 ? String(d.coveredParking) : '' },
      { label: 'Open Parking',         value: d.openParking != null && d.openParking > 0 ? String(d.openParking) : '' },
      { label: 'Total Floors',         value: d.totalFloors   || '' },
      // Commercial
      { label: 'Min Seats',            value: d.minSeats      || '' },
      { label: 'Max Seats',            value: d.maxSeats      || '' },
      { label: 'Cabins',               value: d.numCabins     || '' },
      { label: 'Meeting Rooms',        value: d.numMeetingRooms || '' },
      { label: 'Washroom',             value: d.washroom      || '' },
      { label: 'Conference Room',      value: d.conferenceRoom || '' },
      { label: 'Reception Area',       value: d.receptionArea || '' },
      { label: 'Pantry',               value: d.pantryType    || '' },
      { label: 'Furnishing',           value: d.furnishingAvail || d.furnishing || '' },
      { label: 'Central AC',           value: d.centralAC     || '' },
      { label: 'Floor No.',            value: d.floorNo       || '' },
      { label: 'Lifts',                value: d.lifts         || '' },
      { label: 'Parking',              value: d.commercialParking || '' },
    ].filter(x => x.value),
